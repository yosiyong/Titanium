...
var getRssText = function(item, key) {
    return null == item.getElementsByTagName(key).item(0) ? "pubDate" == key ? getRssText(item, "dc:date") : "" : item.getElementsByTagName(key).item(0).textContent;
};
...
var getLocalDate = function(dateString) {
    ...
    return String.format("%04d-%02d-%02d %02d:%02d", Number(dateParts[3]), Number(MONTH_MAP[dateParts[1].toUpperCase()]), Number(dateParts[2]), Number(timeParts[0]), Number(timeParts[1]));
};
